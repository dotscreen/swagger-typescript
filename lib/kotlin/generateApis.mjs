import { getDefineParam, getQueryParamString, getDefinitionBody, getHeaderParamString, getKotlinType, getSchemaName, } from "./utils.mjs";
...
function generateApis(apis, types, config) {
    ...
    try {
        const schemasMap = new Map(types
            .filter(({ schema }) => Boolean(schema))
            .map(({ name, schema }) => [getSchemaName(name), schema]));
        const apisCode = apis
            ...
            const functionParams = [
                ...pathParams.map(({ name, required, schema, description }) => getDefineParam(name, required, schema, config, description, schemasMap)),
                ...(requestBody
                    ? [
                        getDefinitionBody("requestBody", requestBodyRequired, requestBody, config, undefined, schemasMap),
                    ]
                    : []),
                ...queryParameters.map(({ name, required, schema, description }) => getQueryParamString(name, required, getKotlinType(schema, config, schemasMap), description)),
                ...(headerParams || []).map(({ name, required, description, schema }) => getHeaderParamString(name, required, getKotlinType(schema, config, schemasMap), description)),
            ];
            return (prev +
                `${getJsdoc({
                    description: summary,
