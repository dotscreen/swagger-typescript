...
function generateApis(apis, types, config) {
    ...
    try {
        ...
        const apisCode = apis
            ...
            .reduce((prev, { contentType, summary, deprecated, serviceName, queryParamsTypeName, pathParams, requestBody, requestBodyRequired, headerParams, isQueryParamsNullable, isHeaderParamsNullable, responses, method, endPoint, pathParamsRefString, additionalAxiosConfig, security, }) => {
            return (prev +
                `
${getJsdoc({
                    description: summary,
    ...
    ${
                /** Path parameters */
                pathParams
                    .map(({ name, required, schema, description }) => getDefineParam(name, required, schema, config, description, schemasMap))
                    .join(",")}${pathParams.length > 0 ? "," : ""}${
                /** Request Body */
                requestBody
                    ? `${getDefineParam("requestBody", requestBodyRequired, requestBody, config, undefined, schemasMap)},`
                    : ""}${
                /** Query parameters */
                queryParamsTypeName
...
): Promise<SwaggerResponse<${responses ? getTsType(responses, config, schemasMap) : "any"}>> => {
  ${deprecated
                    ? `
  if (__DEV__) {
    console.warn(
