...
function generateSingleHook(api, context, schemasMap) {
    ...
    const hookConfig = buildHookConfig(api, isQuery, isInfiniteQuery, context, schemasMap);
...
function buildHookConfig(api, isQuery, isInfiniteQuery, context, schemasMap) {
...
function buildVariables(api, context) {
    ...
    api.pathParams.forEach(({ name, required, schema, description }) => {
        parts.push(getDefineParam(name, required, schema, context.config, description, context.schemasMap));
    });
    // Request body
    if (api.requestBody) {
        parts.push(getDefineParam("requestBody", api.requestBodyRequired, api.requestBody, context.config, undefined, context.schemasMap));
    }
    // Query parameters
    if (api.queryParamsTypeName) {
...
function buildHookParams(api, isQuery, isInfiniteQuery, TData, TQueryFnData, TError, context) {
    const params = [];
    if (isQuery) {
        const variables = buildVariables(api, Object.assign(Object.assign({}, context), { config: {}, hasInfiniteQuery: false, hasMutationWithoutVariables: false }));
