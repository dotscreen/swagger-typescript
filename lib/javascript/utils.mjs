...
function getDefineParam(name, required = false, schema, config, description, schemasMap) {
    return getParamString(name, required, getTsType(schema, config, schemasMap), description);
}
...
function appendNullToTsType(type) {
    return type
        .split("|")
        .map((part) => part.trim())
        .includes("null")
 ...
 */
function handleRefType($ref, schemasMap, nullable = false) {
...
function getTsType(schema, config, schemasMap, currentTypeName) {
    ...
    const resolvedType = result || TYPES[type] || "any";
