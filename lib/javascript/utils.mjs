...
function getObjectType(parameter, config, schemasMap) {
    const object = parameter
        ...
        return `${prev}${getJsdoc(Object.assign(Object.assign({}, schema), { deprecated: deprecated || deprecatedMessage ? deprecatedMessage : undefined, example }))}"${name}"${optional ? "?" : ""}: ${getTsType(schema, config, schemasMap)}${nullable ? " | null" : ""};`;
    }, "");
